...
- `cli.py` — entry point, argv dispatch. `handle_ai_prompt(text, commands)` = ask handler. `RESERVED_NAMES` = reserved subcommand names (also duplicated in `commands_config.py` and `web/server.js` — keep all three in sync). Subcommands: `config`, `ai-config`, `ai-clear`, `ai-drop-from`, `playnite-config`, `spotify-config`, `spotify-login`, `memory-config`, `everything-config` (ours), `tools-list`, `tool-run`, `tool-preview`, `tool-safety-set`, `conv-new/list/show/switch/delete`.
...
- `everything_tools.py` + `everything_config.py` (ours) — voidtools' Everything SDK wrapper, Windows-only, ctypes (`Everything64.dll`/`32.dll`, picked by Python's bitness not the OS's). Everything.exe must already be running (IPC client only, not an indexer). `search_files(arguments)`: `query` (required, Everything's own search syntax — `ext:`, `path:`, `size:`, etc.), `max_results`/`match_case`/`match_whole_word`/`match_path`/`regex`/`sort`; returns `{ok, query, count, total_matches, truncated, results:[{path,name,is_folder,size_bytes,date_modified}]}`. `reveal_in_explorer`/`open_file_location(arguments)`: `path` only, shell out to `explorer /select,` / `os.startfile` on the containing folder. All three degrade to `{"error": ...}` off-Windows, DLL-not-found, or Everything-not-running — never raise. DLL resolution: `dll_path` in `~/.jarvis/everything.json` (via `jarvis everything-config`) → `EVERYTHING_SDK_DLL` env var → a short list of common SDK-zip install dirs. Neither tool is in `DEFAULT_CONFIRM_REQUIRED` — read-only search plus opening Explorer, same risk tier as `screenshot_tools`. Web console only: the debug dashboard's `search_files` response gets a bespoke renderer (`debugRenderSearchFilesResponse` in `app.js`) with a Reveal/Open button per row, wired straight to `Api.runTool` — the two Explorer tools work anywhere (CLI, ask), the buttons are just a UI convenience.
...
9. Debug menu centering fix (`.debug-overlay` `flex-end`→`center`), ask-submit silent-block fix (added toast instead of silent `return` when `state.running` blocks a send from a different conversation)
10. **This session:** Everything SDK integration — `search_files`/`reveal_in_explorer`/`open_file_location` (`everything_tools.py` + `everything_config.py`), `everything-config` subcommand, debug-dashboard result renderer with per-row Reveal/Open buttons
