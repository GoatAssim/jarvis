...
def run_scheduler_command(argv):
    ...
    try:
        ...
        if cmd == "notify-history":
            # Full durable inbox, newest first, regardless of what's already
            # been seen by any consumer — notify-list drains the unread
            # queue (and server.js's pushNotifications acks everything it
            # forwards), so it can never answer "what has Jarvis ever sent
            # me". This is the one place that can: notifier.history() reads
            # the same ~/.jarvis/notifications.json inbox without touching
...
def main():
    ...
    from .workspace_cli import COMMANDS as _WORKSPACE_COMMANDS
    if argv[0] in _WORKSPACE_COMMANDS:
        # Daemons, raw log-file search, the backlog, ambient monitoring and
        # onboarding — same split-out reasoning as channels_cli.py below.
        from . import workspace_cli as _workspace_cli
        _workspace_cli.handle(argv)
        return
    ...
    # -------------------------------------------------------------------
    # Part E \u2014 the console store (~/.jarvis/console/<conv_id>.jsonl, see
    # console_store.py). Three non-interactive, JSON-emitting commands,
    # same "web UI shells out, never reads ~/.jarvis itself" pattern as
    # logs-show/logs-clear/logs-append-run just above \u2014 see server.js's
    # /api/console/:id routes.
    # -------------------------------------------------------------------

    if argv[0] == "console-append-run":
        # Direct/live command runs (server.js's "run" websocket \u2014 the
        # Command Builder / Live Output console panel), called in small
        # batches WHILE the run is still going, not once at exit (E.3
        # table row 1 / E.4 point 2). Supersedes logs-append-run's role
        # for this specific case: logs-append-run is left as-is above for
        # whatever else still calls it, but server.js's runOnExit now
        # calls this instead, several times per run.
        #
        # Unlike ai_client.ask() (one process per turn, so console_store's
        # active-context layer fits), a run's batches are each their own
        # short-lived `python -m jarvis` process with nothing remembered
        # between them \u2014 so this always goes through the stateless
        # `console_store.append()` layer, with the turn id server.js
        # itself mints once per run and passes on every batch.
        ...
        try:
            ...
            print(json.dumps({"error": "malformed payload"}))
            sys.exit(1)
        turn = str(payload.get("turn") or "").strip()
        if not turn:
            print(json.dumps({"error": "missing turn id"}))
            sys.exit(1)
        cmdline = payload.get("cmdline")
        if cmdline:
            console_store.append(conv_id, "command", str(cmdline)[:2000], surface="live", turn=turn)
        raw_lines = payload.get("lines")
        if not isinstance(raw_lines, list):
            raw_lines = []
        for item in raw_lines[:5000]:
            if isinstance(item, dict):
                ...
                stream, text = "out", item
            console_store.append(
                conv_id, "stdout" if stream == "out" else "stderr",
                str(text) if text is not None else "", surface="live", turn=turn, stream=stream,
            )
        exit_code = payload.get("exit_code")
        signal_name = payload.get("signal")
        if exit_code is not None or signal_name:
            status_text = f"exit {signal_name}" if signal_name else f"exit {exit_code}"
            console_store.append(conv_id, "status", status_text, surface="live", turn=turn)
        print(json.dumps({"ok": True}))
        return

    if argv[0] == "console-read":
        # GET /api/console/:id's backend. Flags: --since N (exclusive
        # cursor), --kinds a,b,c (comma-separated, see console_store.KINDS),
        # --turn T, --limit N, --after-last-clear (auto-raise --since to
        # the most recent Clear marker \u2014 what the Live Feed's own reload
        # uses; the Ask console's reload leaves this off so a clear there
        # never hides history the user didn't ask to hide).
        from . import console_store
        from . import conversations
        conv_id = argv[1].strip() if len(argv) > 1 else ""
        if not conversations.is_valid_id(conv_id):
        ...
        while i < len(rest):
            flag = rest[i]
            if flag == "--since" and i + 1 < len(rest):
                try:
                    since_seq = int(rest[i + 1])
                except ValueError:
                    pass
                i += 2
            elif flag == "--kinds" and i + 1 < len(rest):
                kinds = [k.strip() for k in rest[i + 1].split(",") if k.strip()]
                i += 2
            elif flag == "--turn" and i + 1 < len(rest):
                turn = rest[i + 1]
                i += 2
            elif flag == "--surface" and i + 1 < len(rest):
                surface = rest[i + 1].strip() or None
                i += 2
            elif flag == "--limit" and i + 1 < len(rest):
                try:
                    limit = int(rest[i + 1])
                except ValueError:
                    pass
        ...
        if result.get("legacy"):
            ...
            # adapting the old command_run log entries so a conversation
            # from before this feature still replays something.
            result["lines"] = console_store.read_legacy_command_run(conv_id)
        print(json.dumps({"id": conv_id, **result}, indent=2))
        return

    if argv[0] == "console-clear":
        # The Live Feed's "Clear" button. Never deletes \u2014 see
        # console_store.clear_marker's docstring \u2014 just records where
        # "cleared" means "everything up to here", so a reload matches
        # what the click did (E.4 point 7).
        from . import console_store
        from . import conversations
        conv_id = argv[1].strip() if len(argv) > 1 else ""
        if not conversations.is_valid_id(conv_id):
            print(json.dumps({"error": "invalid conversation id"}))
            sys.exit(1)
        seq = console_store.clear_marker(conv_id)
        if seq is None:
            print(json.dumps({"error": "couldn't write clear marker"}))
            sys.exit(1)
        print(json.dumps({"ok": True, "cleared_through_seq": seq}))
        return
