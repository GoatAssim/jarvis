...
### Plugin
- .NET Framework 4.6.2, Playnite SDK (project reference to fork's `source/PlayniteSDK` for `GameAction.Id` support)
- Plugin ID: `PlayniteBridge_f47ac10b`
- Auth token auto-generated on first run
- Main plugin: `src/PlayniteBridgePlugin.cs`
- Services: `src/Services/` (GameQueryService, EvalService, PluginIntegrationService, GameSerializationService, **GameActionLaunchService**, SyncClient, SyncEngine, SyncState, BackendDiscovery)
