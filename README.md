...
- Desktop control (`type_text`, `press_key`, `hotkey`, `scroll`,
  `move_mouse`, `click`, `drag`, `get_screen_size`, `get_mouse_position`,
  `list_windows`, `focus_window`, `get_active_window`) needs `pyautogui`
  (which brings `pygetwindow` along with it), *not* installed by default —
  run `pip install -e .[desktop]` (or `pip install pyautogui` directly)
  from `jarvis-cli/` to enable it. Without it, those tools return a clear
  "pyautogui isn't installed" error. The ones that actually move the mouse
  or type (`type_text`, `press_key`, `hotkey`, `click`, `drag`) default to
  `confirm_required=True` in `tool_safety.json` — see the Debug dashboard
  to change that.
- `click_on_text` (find text on screen via OCR and click it) needs
  `pip install -e .[ocr]` (`pytesseract` + `Pillow`) **and** the Tesseract
  OCR binary itself installed and on `PATH` — pytesseract only wraps the
  binary, it doesn't bundle it. On Windows, get it from
  ...
  Without the pip packages you get a clear "pytesseract/Pillow not
  installed" error; with those installed but no binary on `PATH`, a
  separate "Tesseract binary not found" error — so it's obvious which half
  is missing. The screenshot it uses is captured to a temp file, OCR'd,
  and deleted immediately; it's never saved or shown to the user, and
  never sent to the model. It defaults to both `confirm_required=True`
  and `ai_review=True`, same as `run_custom_command`, since it clicks
  whatever text looks closest to what was asked for.
